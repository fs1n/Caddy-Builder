...
echo "PASS: build-caddy.yml exists"

if [ ! -f .github/workflows/check-updates.yml ]; then
    echo "FAIL: check-updates.yml not found"
    exit 1
fi
echo "PASS: check-updates.yml exists"

# Test 2: YAML syntax validation
echo ""
echo "Test 2: Validating YAML syntax"
python3 -c "import yaml; yaml.safe_load(open('.github/workflows/build-caddy.yml'))" && echo "PASS: build-caddy.yml has valid YAML syntax"
python3 -c "import yaml; yaml.safe_load(open('.github/workflows/check-updates.yml'))" && echo "PASS: check-updates.yml has valid YAML syntax"
...
if grep -q "matrix:" .github/workflows/build-caddy.yml; then
    echo "PASS: build-caddy.yml has build matrix"
else
...
for arch in amd64 arm64 armv7 armv6 386; do
    if grep -q "$arch" .github/workflows/build-caddy.yml; then
        echo "PASS: build-caddy.yml includes $arch architecture"
    else
...
if grep -q "xcaddy" .github/workflows/build-caddy.yml; then
    echo "PASS: build-caddy.yml includes xcaddy installation"
else
...
if grep -q "plugins.txt" .github/workflows/build-caddy.yml; then
    echo "PASS: build-caddy.yml reads from plugins.txt"
else
...
if grep -q "schedule:" .github/workflows/check-updates.yml; then
    echo "PASS: check-updates.yml has scheduled runs"
else
...
if grep -q "caddyserver/caddy" .github/workflows/check-updates.yml; then
    echo "PASS: check-updates.yml checks Caddy updates"
else
...
if grep -q "plugins.txt" README.md; then
    echo "PASS: README.md documents plugins.txt"
else
...
if grep -q "upload-artifact@v4" .github/workflows/build-caddy.yml; then
    echo "PASS: build-caddy.yml uses artifact upload"
else
...
if grep -q "test:" .github/workflows/build-caddy.yml; then
    echo "PASS: build-caddy.yml includes test job"
else
...
if grep -qi "version" .github/workflows/build-caddy.yml; then
    echo "PASS: build-caddy.yml tests binary execution"
else
...
if grep -q "create-release:" .github/workflows/build-caddy.yml; then
    echo "PASS: build-caddy.yml includes release job"
else
...
echo ""
echo "Summary:"
echo "  PASS: Workflow files are present and valid"
echo "  PASS: All Linux architectures are covered"
echo "  PASS: Plugin configuration is set up"
echo "  PASS: Update checking is configured"
echo "  PASS: Testing is included"
echo "  PASS: Release automation is configured"
